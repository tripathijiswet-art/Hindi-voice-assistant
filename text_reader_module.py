...
class TextReader:
    def __init__(self, speak, speak_and_wait, stop_tts_immediately, search_root_dir):
        """
        speak(text)             -> non-blocking TTS call (main will handle)
        speak_and_wait(text)    -> blocking TTS call (main will handle)
        stop_tts_immediately()  -> kills current TTS + clears queue
        search_root_dir         -> root folder to scan for .txt files (recursive)
        """
        self.speak = speak
    ...
    def suggest_files(self, spoken_name: str, max_suggestions=3):
        """
        Suggest closest matching file names.
        Returns a list of file-name-keys (not paths).
        """
    ...
    def pause(self):
        if self.reading_active.is_set():
            ...
            self.speak_and_wait("ठीक है! pause कर दिया। resume बोलो ")
            return True
        return False
    ...
    def resume(self):
        if self.reading_active.is_set():
            ...
            self.speak_and_wait("ठीक है! resume कर दिया ")
            return True
        return False
    ...
    def stop_silent(self):
        """Stop without speaking anything (useful for exit)."""
        if self.reading_active.is_set():
            self.stop_event.set()
            self.pause_event.clear()
    ...
    def start_read_by_name(self, spoken_name: str):
        ...
        if not path:
            self.speak_and_wait(f"{spoken_name} के बारे मे जानकारी नहीं मिली ")
            return False

        if all_paths and len(all_paths) > 1:
            self.speak("एक से ज़्यादा same नाम की फाइल मिली हैं। पहली वाली पढ़ रही हूँ।")
    ...
    def _read_worker(self, filepath: str, display_name: str):
        ...
        try:
            ...
            if not chunks:
                self.speak("जानकारी नहीं मिली")
                return

            self.speak(f"जानकारी में दिया गया है")
            ...
            for chunk in chunks:
                ...
                # --- 2. PAUSE LOOP ---
                # If pause_event is set, this thread sits here doing nothing
                # until you call resume() which clears the event.
                ...
                # --- 3. START SPEAKING (NON-BLOCKING) ---
                self.speak(chunk)

                # --- 4. SMART WAIT (The Magic Fix) ---
                # Instead of speak_and_wait, we calculate how long the speech might take.
                ...
                while time.time() - start_time < estimated_duration:
                    # If user says STOP while speaking this chunk
                    if self.stop_event.is_set():
                        self.stop_tts_immediately()  # Physically kill the sound
                        return

                        # If user says PAUSE while speaking this chunk
                    if self.pause_event.is_set():
                        self.stop_tts_immediately()  # Stop the current sound
                        # The loop will catch the pause at the top of the next iteration
                        break

                    time.sleep(0.1)  # Check flags every 100ms
